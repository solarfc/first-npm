...
const modalArr = [].slice.call(modal); //нарезаем блок modal

modalArr.forEach((elem)=> { //для каждого элемента modalArr выделяем элементы
    let elemsArr = elem.childNodes; // забиваем в elemsArr дочерние элементы modalArr
    let button = elem.childNodes[0]; // выводим в button первый дочерний элемент
    let text = elem.childNodes[1]; // выводим в text второй дочерний элемент
    // console.log(text,button);

    button.addEventListener('click',function(e){
        let bool = text.classList.contains('close');
        if (bool === false) {
